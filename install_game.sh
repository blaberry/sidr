...
if [[ $? -eq 0 ]]; then
  docker run --rm --name steamcmd -v $name:/game steamcmd /home/steam/steamcmd/steamcmd.sh @sSteamCmdForcePlatformType windows +login anonymous +force_install_dir /game +app_update $appid validate +quit
fi
